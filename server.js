...
server.use((req, res, next) => {
  ...
  // Handle /attendee endpoint
  else if (req.path === '/attendee' && req.method === 'GET') {
    const db = router.db;
    let attendees = db.get('attendees').value();
    ...
    if (startDate || endDate) {
      attendees = attendees.filter(attendee => {
        const lastActivity = attendee.lastliveactivity;
        if (!lastActivity) return false;
    ...
    Object.keys(queryParams).forEach(key => {
      if (key !== '_sort' && key !== '_order') {
        attendees = attendees.filter(attendee => {
          ...
          return attendee[key] == queryParams[key];
        });
      }
    });
    ...
    const pageCount = Math.ceil(totalattendees / pageSize);
    const currentPage = pageNumber;
    const maxValidPage = pageCount - 1;
